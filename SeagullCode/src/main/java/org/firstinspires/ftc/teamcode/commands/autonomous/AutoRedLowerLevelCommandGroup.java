...
public class AutoRedLowerLevelCommandGroup extends SequentialCommandGroup {
    public AutoRedLowerLevelCommandGroup(DrivebaseSubsystem drive, DumpSubsystem bucket, IntakeSubsystem intake) {
        super(
                ...
                new TrajectorySequenceCommand(drive, AutonomousConstants.RED_ALLIANCE_HUB_LEVEL1_TO_DEPOT_COLLECT1),

                new TrajectorySequenceCommand(drive, AutonomousConstants.RED_DEPOT_COLLECT1_TO_ALLIANCE_HUB_LEVEL1).alongWith(new IntakeOutCommand(intake).sleep(0.5).andThen(new IntakeStopCommand(intake))),
                new DumpUnloadTopLevelCommand(bucket).withTimeout(1.5),
                new WaitCommand(0.3),
                new DumpCollectCommand(bucket), // Bucket dump command
                new IntakeInCommand(intake), // Intake command - spin the intake before arrived at the depot
                new TrajectorySequenceCommand(drive, AutonomousConstants.RED_ALLIANCE_HUB_LEVEL1_TO_DEPOT_COLLECT2),
