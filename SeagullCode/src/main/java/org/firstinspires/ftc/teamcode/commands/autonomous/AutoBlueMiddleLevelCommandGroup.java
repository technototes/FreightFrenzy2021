...
public class AutoBlueMiddleLevelCommandGroup extends SequentialCommandGroup {
    public AutoBlueMiddleLevelCommandGroup(DrivebaseSubsystem drive, DumpSubsystem bucket, IntakeSubsystem intake) {
        super(
                ...
                new TrajectorySequenceCommand(drive, AutonomousConstants.BLUE_ALLIANCE_HUB_LEVEL2_TO_DEPOT_COLLECT), // Different park command

                new TrajectorySequenceCommand(drive, AutonomousConstants.BLUE_DEPOT_COLLECT_TO_ALLIANCE_HUB_LEVEL_2).alongWith(new IntakeOutCommand(intake).sleep(0.5).andThen(new IntakeStopCommand(intake))),
                new DumpUnloadTopLevelCommand(bucket).withTimeout(1.5),
                new WaitCommand(0.3),
                new DumpCollectCommand(bucket), // Bucket dump command
                new IntakeInCommand(intake), // Intake command - spin the intake before arrived at the depot
                new TrajectorySequenceCommand(drive, AutonomousConstants.BLUE_ALLIANCE_HUB_LEVEL2_TO_DEPOT_COLLECT),
