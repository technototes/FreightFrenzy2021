...
public class Controls {
    ...
    public void bindDepositControls() {
        ...
        if(INTAKE_CONNECTED) toIntakeButton.whenPressed(new ArmRetractCommand(robot.depositSubsystem).andThen(new IntakeInCommand(robot.intakeSubsystem)));
        else toIntakeButton.whenPressed(new ArmRetractCommand(robot.depositSubsystem));
    ...
    public void bindLiftControls() {
        neutralHubButton.whenPressed(new WaitCommand(0.3).andThen(new LiftNeutralCommand(robot.liftSubsystem).withTimeout(1.5)));
        specificHubButton.whenPressed(new WaitCommand(0.3).andThen(new LiftLevel3Command(robot.liftSubsystem).withTimeout(1.5)));
    ...
    public void bindIntakeControls() {
        if(!DEPOSIT_CONNECTED) toIntakeButton.whenPressed(new WaitCommand(1).andThen(new IntakeSafeCommand(robot.intakeSubsystem)));
